...
CREATE TABLE Shows (
    ...
    StationID INT,
    LaunchDate DATE,
    CONSTRAINT fk_Shows_HostID FOREIGN KEY (HostID)
        REFERENCES Hosts (HostID),
    CONSTRAINT fk_Shows_StationID FOREIGN KEY (StationID)
...
CREATE TABLE ShowPartnerships (
    ShowID INT,
    PartnershipID INT,
    CONSTRAINT fk_ShowPartnerships_ShowID FOREIGN KEY (ShowID)
        REFERENCES Shows (ShowID),
    CONSTRAINT fk_ShowPartnerships_PartnershipID FOREIGN KEY (PartnershipID)
        REFERENCES Partnerships (PartnershipID)
);
...
CREATE TABLE Awards (
    ...
    Year INT,
    ShowID INT,
    CONSTRAINT fk_Awards_ShowID FOREIGN KEY (ShowID)
        REFERENCES Shows (ShowID)
);
...
FROM
    stations;
SELECT 
    StationID, StationName, Location, Frequency, LaunchDate
FROM
    stations
WHERE
    Location = 'East';
...
SELECT 
    s.ShowID, h.HostName, s.ShowName
FROM
    shows s
        JOIN
    hosts h ON s.HostID = H.HostID
WHERE
    HostName = 'Vrajesh Hirjee';

-- 3)Count the number of awards each show has won?
SELECT 
    s.ShowID, s.ShowName, COUNT(AwardID) AS Num_of_awards
FROM
    Awards a
        JOIN
    shows s ON a.ShowID = s.ShowID
GROUP BY ShowName , ShowID;

-- 4)Find shows that have partnerships with "Spotify"?
SELECT 
    s.ShowID, s.ShowName, p.partnername
FROM
    shows s
        JOIN
    showpartnerships sp ON s.ShowID = sp.ShowID
        JOIN
    partnerships p ON p.partnershipID = sp.partnershipID
WHERE
    partnerName = 'spotify';

-- 5)Retrieve hosts who joined before 2010?
SELECT 
    HostID, HostName, JoinDate
FROM
    Hosts
WHERE
    YEAR(joinDate) < 2010;
...
SELECT 
    HostID, HostName, SUM(ShowCount) AS Total_shows
FROM
    Hosts
